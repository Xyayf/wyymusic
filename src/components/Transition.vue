        ...
        <div class="mlsi">
          <ul>
            <li
              ...
              :key="item.data.vid"
              @click="playdetail(item.data.vid, item.data.coverUrl, 3)"
            >
  ...
  methods: {
    getvideoList () {
      ...
      this.$axios({
        method: 'get',
        dataType: 'json',
        url: Config.api.getvideoList + "?cookie=" + this.$store.state.userinfo.cookie,
      }).then(res => {
    ...
    gettypemoves () {
      ...
      this.$axios({
        method: 'get',
        dataType: 'json',
        url: Config.api.gettypemove + this.vid + "&cookie=" + this.$store.state.userinfo.cookie,
      }).then(res => {
