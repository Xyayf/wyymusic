          ...
          <ul>
            <li style="color: #fff" v-for="(item, index) in listData">
              <div class="ingandname">
                <p>
                  <img width="20" height="20" v-lazy="item.user.avatarUrl" />
                </p>
  ...
  methods: {
    ...
    getVideo (mvid) {
      this.$axios({
        method: 'get',
        dataType: 'json',
        url: Config.api.getmvDetail + mvid + "&cookie=" + this.$store.state.userinfo.cookie,
      }).then(res => {
        var moveyrl = [{
          type: "video/mp4",
    ...
    pl () {
      this.$axios({
        method: 'get',
        dataType: 'json',
        url: Config.api.getMvpl + this.moveId + "&offset=" + this.page,
      }).then(res => {
