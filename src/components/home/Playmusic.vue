      ...
      <div class="lrcData">
        <ul id="topmian">
          <li
            ...
            :style="{ color: index == currentRow ? 'red' : '' }"
          >
            {{ item.text }}
          </li>
        </ul>
  ...
  methods: {
    ...
    playMusic () {
      ...
      this.$axios({
        method: "get",
        dataType: "json",
        url: Config.api.playUrl + this.$route.query.musicId,
      })
        .then((res) => {
    ...
    getblc () {
      this.$axios({
        method: "get",
        dataType: "json",
        url: Config.api.getblc + this.$route.query.musicId,
      })
        .then((res) => {
    ...
    lyricCurrent (newName, oldName) {
      this.lyric.forEach((element, index) => {
        if (this.lyricCurrent == element.time) {
          document.getElementById("topmian").style.marginTop =
            -index * 30 + "px"
